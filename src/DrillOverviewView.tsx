          ...
          <For each={props.section.lines}>
            {(line) => (
              <Flex direction="row" wrap="wrap" gap="1" justify="center">
                <For each={line.measures}>{(measure) => <MeasureView measure={measure} />}</For>
              </Flex>
            )}
          </For>
        </Flex>
...
function RowView(props: { row: Row }) {
  return (
    <Flex direction="row" wrap="wrap" gap="2" justify="center">
      <For each={props.row}>{(section) => <OverviewSectionView section={section} />}</For>
    </Flex>
  )
}
...
export function DrillOverView(props: { drill: Drill }) {
  return (
    <Flex direction="column" gap="4" align="center" style={{ "max-width": "1200px" }}>
      <For each={props.drill.rows}>{(row) => <RowView row={row} />}</For>
    </Flex>
  )
}
